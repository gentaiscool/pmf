...
num_train = int(0.8 * total_sample)
num_test = int(0.2 * total_sample)
...
pmf = PMF(K=5, lr=0.025, lambda_u=0.1, lambda_v=0.1, max_epoch=20, batch_size=250, num_user=num_user, num_item=num_item, verbose=False)
...
if args.task == "task1":
    ...
    parameters = {'K':[1,2,3,4,5], 'lambda_u':[0.1], 'lambda_v':[0.1]}
    clf = GridSearchCV(pmf, parameters, scoring=rmse, cv=5, verbose=1)
    clf.fit(train, train_ratings)
    ...
    print("e.g. python main_dense.py --task=task1")